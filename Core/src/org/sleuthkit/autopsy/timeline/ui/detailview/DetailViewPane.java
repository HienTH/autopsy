...
public class DetailViewPane extends AbstractTimelineChart<DateTime, EventStripe, EventNodeBase<?>, DetailsChart> {
    ...
    @NbBundle.Messages("DetailViewPane.primaryLaneLabel.text=Tất cả sự kiện (Đã lọc)")
    ...
    static private class DetailViewSettingsPane extends HBox {
        ...
        @NbBundle.Messages({
            "DetailViewPane.truncateSliderLabel.text=chiều rộng mô tả tối đa (px):",
            "DetailViewPane.advancedLayoutOptionsButtonLabel.text=Tùy chọn Giao diện Nâng cao",
            "DetailViewPane.bandByTypeBox.text=Nhóm theo loại",
            "DetailViewPane.oneEventPerRowBox.text=Một trên mỗi hàng",
            "DetailViewPane.truncateAllBox.text=Miêu tả cắt ngắn",
            "DetailViewPane.showRadio.text=Hiển thị Mô tả đầy đủ",
            "DetailViewPane.countsRadio.text=Chỉ hiển thị số lần",
            "DetailViewPane.hiddenRadio.text=Ẩn Mô tả"})
        @FXML
        void initialize() {
    ...
    @NbBundle.Messages({
        "DetailViewPane.loggedTask.queryDb=Truy xuất dữ liệu sự kiện",
        "DetailViewPane.loggedTask.name=Đang cập nhật chế độ xem chi tiết",
        "DetailViewPane.loggedTask.updateUI=Populating view",
        "DetailViewPane.loggedTask.continueButton=Tiếp tục",
        "DetailViewPane.loggedTask.backButton=Trở lại (Hủy)",
        "# {0} - number of events",
        "DetailViewPane.loggedTask.prompt=Bạn sắp hiển thị chi tiết cho {0} sự kiện. Điều này có thể rất chậm và có thể làm mất bộ nhớ sẵn có. \\n \\ nBạn có muốn tiếp tục không?"})
