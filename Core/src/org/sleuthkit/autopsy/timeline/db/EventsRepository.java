...
public class EventsRepository {
    ...
    private class DBPopulationWorker extends CancellationProgressTask<Void> {
        ...
        @Override
        @NbBundle.Messages({"progressWindow.msg.refreshingFileTags=Làm mới các thẻ tập tin",
            "progressWindow.msg.refreshingResultTags=Làm mới các thẻ kết quả",
            "progressWindow.msg.gatheringData=Thu thập dữ liệu sự kiện",
            "progressWindow.msg.commitingDb=Ủy thác cơ sở dữ liệu sự kiện"})
        ...
        @NbBundle.Messages("progressWindow.msg.populateMacEventsFiles=Nạp các sự kiện thời gian MAC cho các tệp")
        ...
        @Override
        @NbBundle.Messages("msgdlg.problem.text=Đã xảy ra sự cố khi điền vào biểu đồ thời gian."
                + "  Không phải tất cả các sự kiện có thể có mặt hoặc chính xác.")
        protected void done() {
            super.done();
            try {
                get();
         ...
         */
        @NbBundle.Messages({"# {0} - event type ", "progressWindow.populatingXevents=Phổ biến {0} sự kiện"})
