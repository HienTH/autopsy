 ...
 * </ul>
 */
@NbBundle.Messages({"Timeline.dialogs.title= Dòng sự kiện",
    "TimeLinecontroller.updateNowQuestion=Bạn có muốn cập nhật cơ sở dữ liệu sự kiện ngay bây giờ?"})
public class TimeLineController {
     ...
     */
    @NbBundle.Messages({
        "TimeLineController.setEventsDBStale.errMsgStale=Không thể đánh dấu dòng sự kiện là cũ. Một số kết quả có thể đã lỗi thời hoặc thiếu.",
        "TimeLineController.setEventsDBStale.errMsgNotStale=Không thể đánh dấu dòng sự kiện như không lỗi thời. Một số kết quả có thể đã lỗi thời hoặc thiếu."})
    @ThreadConfined(type = ThreadConfined.ThreadType.JFX)
    ...
    @ThreadConfined(type = ThreadConfined.ThreadType.JFX)
    @NbBundle.Messages({
        "TimeLineController.setIngestRunning.errMsgRunning=Không thể đánh dấu dòng sự kiện khi đang điều tra. Một số kết quả có thể đã lỗi thời hoặc thiếu.",
        "TimeLinecontroller.setIngestRunning.errMsgNotRunning=Không thể đánh dấu dòng sự kiện khi điều tra không chạy. Một số kết quả có thể đã lỗi thời hoặc thiếu."})
    ...
    @NbBundle.Messages({"TimeLineController.errorTitle=Lỗi Dòng sự kiện.",
        "TimeLineController.outOfDate.errorMessage=Lỗi xác định xem thời gian đã lỗi thời. Chúng tôi sẽ giả định rằng nó nên được cập nhật. Xem nhật ký để biết thêm chi tiết.",
        "TimeLineController.rebuildReasons.outOfDateError=Không thể xác định được liệu dữ liệu thời gian đã lỗi thời.",
        "TimeLineController.rebuildReasons.outOfDate=Dữ liệu sự kiện đã lỗi thời: Không phải tất cả sự kiện sẽ được hiển thị.",
        "TimeLineController.rebuildReasons.ingestWasRunning=Cơ sở dữ liệu sự kiện trước đây đã được nhập vào khi đang chạy: Một số sự kiện có thể bị thiếu, không đầy đủ hoặc không chính xác.",
        "TimeLineController.rebuildReasons.incompleteOldSchema=Cơ sở dữ liệu Sự kiện đã được điền trước đó mà không có thông tin không đầy đủ: Một số tính năng có thể không khả dụng hoặc không hoạt động trừ khi bạn cập nhật cơ sở dữ liệu sự kiện."})
