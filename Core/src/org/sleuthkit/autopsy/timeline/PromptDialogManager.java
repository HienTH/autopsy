...
public final class PromptDialogManager {
    ...
    @NbBundle.Messages("PrompDialogManager.buttonType.showTimeline=Tiếp tục")
    ...
    @NbBundle.Messages("PrompDialogManager.buttonType.continueNoUpdate=Tiếp tục mà không cập nhật")
    ...
    @NbBundle.Messages("PrompDialogManager.buttonType.update=Cập nhật DB")
     ...
     */
    @NbBundle.Messages({"PromptDialogManager.progressDialog.title=Lọc dữ liệu Dòng sự kiện"})
    @ThreadConfined(type = ThreadConfined.ThreadType.JFX)
     ...
     */
    @NbBundle.Messages({
        "PromptDialogManager.confirmDuringIngest.headerText=Bạn đang cố gắng cập nhật Timeline DB trước khi nhập xong. DB Timeline có thể không đầy đủ.",
        "PromptDialogManager.confirmDuringIngest.contentText=Bạn có muốn tiếp tục?"})
    @ThreadConfined(type = ThreadConfined.ThreadType.JFX)
     ...
     */
    @NbBundle.Messages({
        "PromptDialogManager.rebuildPrompt.headerText=DB Timeline chưa đầy đủ hoặc lỗi thời."
        + "  Một số sự kiện có thể bị thiếu hoặc không chính xác và một số tính năng có thể không khả dụng.",
        "PromptDialogManager.rebuildPrompt.details=Chi tiết"})
    @ThreadConfined(type = ThreadConfined.ThreadType.JFX)
    ...
    @NbBundle.Messages({
        ...
        + "Có quá nhiều tệp trong DB để đảm bảo hiệu suất hợp lý."
        + "  Dòng thời gian sẽ bị tắt. ",
    ...
    @NbBundle.Messages({
        ...
        + "Chức năng Dòng sự kiện hiện không có sẵn cho Linux."
        + "  Dòng sự kiện sẽ bị tắt. ",
