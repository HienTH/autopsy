...
public final class TimeLineTopComponent extends TopComponent implements ExplorerManager.Provider {
     ...
     */
    @NbBundle.Messages({"TimelineTopComponent.selectedEventListener.errorMsg=Đã xảy ra sự cố khi tải nội dung cho sự kiện đã chọn."})
     ...
     */
    @NbBundle.Messages({
        "TimeLineTopComponent.eventsTab.name=Sự kiện",
        "TimeLineTopComponent.filterTab.name=Bộ lọc"})
    ...
    @NbBundle.Messages({
        ...
        "# {1} - end of date range",
        "TimeLineResultView.startDateToEndDate.text={0} đến {1}"})
