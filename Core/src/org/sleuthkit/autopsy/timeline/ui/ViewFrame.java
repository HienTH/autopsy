...
final public class ViewFrame extends BorderPane {
    ...
    @FXML
    @NbBundle.Messages({
        "ViewFrame.viewModeLabel.text=Chế độ xem:",
        "ViewFrame.startLabel.text=Bắt đầu:",
        "ViewFrame.endLabel.text=:Kết thúc",
        "ViewFrame.countsToggle.text=Đếm",
        "ViewFrame.detailsToggle.text=Chi tiết",
        "ViewFrame.listToggle.text=Danh sách",
        "ViewFrame.zoomMenuButton.text=Phóng to / thu nhỏ",
        "ViewFrame.tagsAddedOrDeleted=Thẻ đã được tạo hoặc bị xóa. Chế độ xem có thể không được cập nhật."
    })
    void initialize() {
    ...
    @NbBundle.Messages({
        "# {0} - datasource name",
        "ViewFrame.notification.newDataSource={0} đã được thêm như là một nguồn dữ liệu mới. DB Timeline có thể đã lỗi thời."})
    ...
    @NbBundle.Messages({
        "# {0} - datasource name",
        "ViewFrame.notification.analysisComplete=Phân tích đã kết thúc cho {0}. DB Timeline có thể đã lỗi thời."})
     ...
     */
    @NbBundle.Messages({"ViewFrame.histogramTask.title=Xây dựng lại Histogram",
        "ViewFrame.histogramTask.preparing=Chuẩn bị",
        "ViewFrame.histogramTask.resetUI=Đặt lại ",
        "ViewFrame.histogramTask.queryDb=Truy vấn FB",
        "ViewFrame.histogramTask.updateUI2=Đang cập nhật giao diện người dùng"})
    ...
    @NbBundle.Messages("NoEventsDialog.titledPane.text=Không có sự kiện hiển thị")
    private class NoEventsDialog extends StackPane {
        ...
        @FXML
        @NbBundle.Messages("ViewFrame.noEventsDialogLabel.text=Không có sự kiện nào hiển thị với cài đặt thu phóng / lọc hiện tại.")
        void initialize() {
    ...
    private class Refresh extends Action {
        ...
        @NbBundle.Messages({
            "ViewFrame.refresh.text=Làm mới Chế độ xem",
            "ViewFrame.refresh.longText=Làm mới chế độ xem để bao gồm thông tin trong DB nhưng không được hiển thị, chẳng hạn như các thẻ mới cập nhật."})
        Refresh() {
